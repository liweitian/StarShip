...
public class GamePanel extends Panel {
    ...
    private  void drawFoe(Graphics g){
        g.setColor(Color.white);
        g.setFont(fnt);
        g.drawString("Foe Remain: "+CommandCenter.getInstance().getMovFoes().size(), nFontWidth, nFontHeight+10);
    }
    ...
    //for each movable array, process it.//把所有的运动物体打出来 这个...是一个符号
    ...
    private void displayTextOnScreen() {
        ...
        grpOff.drawString(strDisplay,
                (Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
                        + nFontHeight + 320);
        strDisplay = "'B' for Bomb";
        grpOff.drawString(strDisplay,
                (Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
                        + nFontHeight + 360);
        strDisplay = "'G' for GuidedMissile";
        grpOff.drawString(strDisplay,
                (Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
                        + nFontHeight + 400);
    }
